...
from tkinter import filedialog  #导入文件库
import time
import os
import ftp as dow
import sys
...
def send(name):
    ...
    ## 聊天窗口
    # 创建图形界面
    #root_send = tkinter.Tk()
    root_send = tkinter.Toplevel()

    
    root_send.title(user)  # 窗口命名为用户名
    root_send['height'] = 390
    root_send['width'] = 580
    root_send.resizable(0, 0)  # 限制窗口大小

    # 创建多行文本框
    listbox = ScrolledText(root_send)
    ...
    def express():
        ...
        if ee == 0:
            ee = 1
            b1 = tkinter.Button(root_send, command=bb1, image=p1,relief=tkinter.FLAT ,bd=0)
            b2 = tkinter.Button(root_send, command=bb2, image=p2,
                                relief=tkinter.FLAT ,bd=0)
            b3 = tkinter.Button(root_send, command=bb3, image=p3,
                                relief=tkinter.FLAT ,bd=0)
            b4 = tkinter.Button(root_send, command=bb4, image=p4,
                                relief=tkinter.FLAT ,bd=0)
    ...
    # 创建表情按钮
    eBut = tkinter.Button(root_send, text='表情', command=express)
    ...
    pBut = tkinter.Button(root_send, text='上传', command=picture)
    ...
    # 创建截屏按钮
    sBut = tkinter.Button(root_send, text='下载', command=down)
    ...
    # 创建文件按钮
    fBut = tkinter.Button(root_send, text='game', command=play)
    fBut.place(x=185, y=320, width=60, height=30)

    # 创建多行文本框, 显示在线用户
    listbox1 = tkinter.Listbox(root_send)  
    ...
    # 查看在线用户按钮
    button1 = tkinter.Button(root_send, text='在线用户', command=users_online)
    button1.place(x=505, y=320, width=70, height=30)

    # 创建输入文本框和关联变量
    a = tkinter.StringVar()
    a.set('')
    entry = tkinter.Entry(root_send, width=120, textvariable=a)
    ...
    # 创建发送按钮
    button = tkinter.Button(root_send, text='发送', command=send)
    button.place(x=515, y=353, width=60, height=30)
    root_send.bind('<Return>', send)  # 绑定回车发送信息
    ...
    def donothing():
        ...
        sys.exit(0)
        #root_send.destroy()
    root_send.protocol("WM_DELETE_WINDOW", donothing)
    #root_send.mainloop()
    #s.send(b'quit')
